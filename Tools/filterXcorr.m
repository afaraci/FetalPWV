function miF = filterXcorr( mi, ord )
...
dFilt = designfilt('lowpassfir', 'FilterOrder',ord, 'PassbandFrequency', CoF, 'StopbandFrequency',CoF*5, 'DesignMethod','ls','SampleRate', 1000);

% dFilt = designfilt('bandpassfir', 'FilterOrder', ord, ....
%     'CutoffFrequency1', 0.000000000000000001, 'CutoffFrequency2', CoF, 'SampleRate', Fs);

miF = filtfilt(dFilt,mi);
% miF = filter( dFilt, mi );
%  gd = grpdelay( dFilt );
% miF = miF( gd(1)+1 : end );
miF = miF + aveSig;